...
// This class has many code smells - you need to identify them and fix them as we did in class
...
public class MyClient implements Runnable {
    ...
    public void run() {
        try {
            ...
            if (!get.startsWith("GET")) {
                p.print(generateErrorPage("Server only accepts GET command."));
                closeAll();
            }
            else {
    ...
    private void downloadFile(String fileLocation) throws IOException {
        ...
        if (f.exists() || f.isDirectory()) {
            ...
            p.print(generateErrorPage("File doesn't exist."));
        }
        closeAll();
    }
    ...
    private Object execJava(String path) throws IOException {
        ...
        if (pathPart.length < 3) {
            p.println(generateErrorPage("Enter fully-qualified-classname after /java."));
            p.flush();
            closeAll();
            return null;
        }
        ...
        try {
            ...
            if (o instanceof ServerSideJava) {
                ...
                p.println(generateErrorPage("Java class " + className + " not ServerSideJava."));
                p.flush();
            }
        }
        catch (ClassNotFoundException c) {
            c.printStackTrace(System.err);
            p.println(generateErrorPage("<p>Cannot execute!!! </p>" + c.toString()));
            closeAll();
            return null;
        }
        catch (InstantiationException i) {
            i.printStackTrace(System.err);
            p.println(generateErrorPage("<p>Cannot execute!!! </p>" + i.toString()));
            closeAll();
            return null;
        }
        catch (IllegalAccessException il) {
            il.printStackTrace(System.err);
            p.println(generateErrorPage("<p>Cannot execute!!! </p>" + il.toString()));
            closeAll();
            return null;
        }
        closeAll();
    ...
    static public String generateErrorPage(String contentIn){
